...
class WhatIfSimulatorWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        self.plot.addLegend()
        self.cur_step = self.plot.plot(pen=pg.mkPen('#57cc99', width=2), name='虚拟阶跃响应')
...
class AdvancedTuningWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        self.interp_mode = QComboBox(); self.interp_mode.addItems(["线性", "阶梯", "平滑阶梯"])
        ...
        self.cur_v = self.plot.plot(pen='y', name='电池电压')
        self.cur_kp = self.plot.plot(pen='c', name='调度Kp')
        self.cur_u = self.plot.plot(pen='m', name='LADRC输出')
    ...
    def update_from_telemetry(self, tel: dict, device_id: int, channel_id: int):
        ...
        self.log.setText(
            f"插值模式={self.interp_mode.currentText()} 模拟={'开启' if self.sim_mode else '关闭'}\n"
            f"原始分段={raw_band} 生效分段={active} 插值分段={band}\n"
            f"电压={voltage:.2f} Kp={gains.kp:.3f} Ki={gains.ki:.3f} Kd={gains.kd:.3f}\n"
            f"LADRC输出={u:.3f} 扰动估计z2={self.ladrc_state.z2:.3f} 基础控制u0={u0:.3f} 扰动补偿={u_comp:.3f}"
        )
        self._last_device = device_id; self._last_channel = channel_id

    def _apply_current(self):
        if not self.last_pick:
            self.log.append("[警告] 无可下发参数")
            return
    ...
    def _apply_pid_candidates(self):
        ...
        for idx, (_, ratio) in enumerate(plan):
            ...
            self.send_frame_requested.emit(frame)
            self.log.append(f"[完成] PID候选已下发 通道={idx} Kp={kp:.3f} Ki={ki:.3f} Kd={kd:.3f}")
    ...
    def _apply_whatif_to_panel(self, ctrl_mode: str, p1: float, p2: float, p3: float):
        if ctrl_mode == "LADRC":
            ...
            self.wo.setValue(p2)
            self.b0.setValue(p3)
            self.log.append(f"[完成] 已应用 LADRC 参数 wc={p1:.2f}, wo={p2:.2f}, b0={p3:.2f}")
        else:
            self.log.append(f"[提示] PID 模拟参数 Kp={p1:.2f}, Ki={p2:.2f}, Kd={p3:.2f}；可用于调整电压分段 PID 增益。")

    def _run_tuning_map(self):
        if self.sim_mode:
            self.log.append("[警告] 请先停止模拟，再生成调参地图")
            return
