...
class AdvancedTuningWindow(QWidget):
    ...
    def _apply_current(self):
        ...
        # 重构：使用 core 层接口生成干净 Payload
        payload = encode_param_set_payload(kp=gains.kp, ki=gains.ki, kd=gains.kd)
        frame = encode_frame(
            FRAME_PARAM_SET, 
            getattr(self, '_last_device', 1), 
            getattr(self, '_last_channel', 0), 
            payload
        )
        # 重构：直接触发信号，主窗口接收后会自动调度 Worker
    ...
    def _run_ladrc_heatmap(self):
        ...
        w = QWidget(); w.setWindowTitle("LADRC 参数热力图(IAE)")
