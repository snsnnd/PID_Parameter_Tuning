...
static void on_param_set(uint8_t device_id, uint8_t channel_id, const pid_debug_param_t *param) {
    if (device_id != GIMBAL_YAW_DEVICE_ID || param == 0) return;
    user_pid_debug_apply_param(channel_id, param);
}

void gimbal_yaw_pid_debug_init(void) {
    pid_debug_port_t port = { user_pid_debug_write, user_pid_debug_time_ms, on_param_set };
    pid_debug_init(&port);
}

void gimbal_yaw_pid_debug_rx_byte(uint8_t byte) { pid_debug_rx_byte(byte); }
void gimbal_yaw_pid_debug_poll(void) { pid_debug_poll(); }

int gimbal_yaw_pid_debug_report(uint8_t channel_id, const pid_debug_channel_sample_t *sample) {
    ...
    return pid_debug_send_telemetry(GIMBAL_YAW_DEVICE_ID, channel_id, &t);
}

int gimbal_yaw_pid_debug_heartbeat(uint16_t status_flags) {
    return pid_debug_send_heartbeat(GIMBAL_YAW_DEVICE_ID, status_flags, user_pid_debug_time_ms());
}
